...
## Fourth-review scoped plan
- Capture an immutable fingerprint of identity and validation-relevant stored context before awaiting reasoning validation (session, folder, provider/vendor, model, effort, historical IDs and branch guard). Re-read by stable chat ID afterward; reject with a retryable context-changed error if that context changed, including creation/deletion during adoption.
- Never identity-upsert an existing record from a discovery snapshot. Use a metadata-only merge after successful context checks, with opt-in legacy JSON normalization; preserve unrelated concurrent metadata.
- Apply the same post-await guard in low-level sendMessage before provenance writes or stopping/resuming a session. Keep this bounded to preflight, not a global concurrency redesign.
- Exercise actual POST/storage races and low-level sendMessage races for all requested context fields plus unrelated title/bookmark updates. Run focused tests, build, changed lint and diff checks; rebase only while validation is idle, then push exact head.

### Fourth-review implementation and validation
- Added an immutable stored-context fingerprint shared by POST preflight and low-level sendMessage. Session/folder/provider/vendor/model/effort/history/branch changes across validation, or record appearance/disappearance, abort before provenance writes or execution; POST returns 409 with code chat_context_changed and retry guidance.
- Existing records now receive only scoped routing metadata merges, never identity-bearing discovery upserts. Opt-in legacy normalization retains the earlier malformed-metadata fix. Concurrent unrelated title/bookmark fields survive.
- Actual POST handler plus real storage regression tests cover rotation, folder, provider, explicit ACP vendor, model, effort, history, branch, deletion, discovery adoption races and unrelated metadata. Low-level sendMessage tests independently exercise those stored-context changes and a successful offline mocked-provider resume preserving unrelated metadata.
- Validation: 151 tests passed across 16 affected suites (maxWorkers=2); npm run build passed; changed-file ESLint had zero errors / 39 warnings; git diff --check passed. No repeated full local suite per request; existing build warnings remain. No paid calls, live-chat actions or global concurrency redesign.
