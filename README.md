...
- **A two-tier memory system** — daily journal files at `memory/YYYY-MM-DD.md` for running notes, and a curated long-term `MEMORY.md` distilled over time. Today's journal and `MEMORY.md` are pre-loaded whole; each *earlier* day is capped by the **journal budget** on the Memory tab (4k tokens by default, or no limit), and a capped journal carries a notice telling the agent to read or search the file for the entries left out
