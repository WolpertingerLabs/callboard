...
export function createNativeDesktopDriver(options: NativeDesktopDriverOptions = {}): Driver {
  ...
  const compatible = () =>
    config.enabled === true &&
    config.acknowledgeFullDesktopAccess === true &&
    config.permissions &&
    ["webAccess", "fileRead", "fileWrite", "codeExecution"].every((k) => config.permissions![k as keyof NonNullable<typeof config.permissions>] === "allow");
  if (config.driver && (config.driver.kind !== "native-desktop" || !config.driver.lockDomain))
    throw new ComputerUseError("invalid_request", "Native helpers require native kind and shared input lockDomain");
  ...
  const run = (binary: "/usr/bin/xdotool" | "/usr/bin/import", args: string[], signal?: AbortSignal): Promise<Buffer> =>
    new Promise((resolve, reject) => {
      if (signal?.aborted) {
        reject(new ComputerUseError("cancelled"));
        return;
      }
      execFile(binary, args, { env, encoding: "buffer", timeout: 5000, maxBuffer: 9 * 1024 * 1024, signal, killSignal: "SIGKILL" }, (error, stdout) => {
        if (error) reject(new ComputerUseError(signal?.aborted ? "cancelled" : "driver_error", "X11 command failed"));
        else resolve(stdout);
      });
    });
  const probe = async (): Promise<Probe> => {
    ...
    return {
      available: true,
      kind: "native-desktop",
      capabilities: ["screenshot", "pointer", "keyboard"],
      reason: "X11 prerequisites detected; live app workflows remain operator-qualified",
    };
  };
  return {
    kind: "native-desktop",
    ...
    async open(context) {
      ...
      const input = (args: string[], sig?: AbortSignal) => {
        if (closed) throw new ComputerUseError("stopped");
        return run("/usr/bin/xdotool", args, sig);
      };
      const releaseInput = async () => {
        for (const b of [...heldButtons]) {
          await input(["mouseup", b]);
      ...
      return {
        ...
        async act(raw: Action, sig) {
          const a = actionSchema.parse(raw);
          sig.throwIfAborted();
          try {
            switch (a.type) {
              case "move":
                await input(["mousemove", String(a.x), String(a.y)], sig);
                break;
              case "click":
                await input(["mousemove", String(a.x), String(a.y)], sig);
                sig.throwIfAborted();
                {
                  const b = String({ left: 1, middle: 2, right: 3 }[a.button ?? "left"]);
                  heldButtons.add(b);
                  await input(["mousedown", b], sig);
                  sig.throwIfAborted();
                  await input(["mouseup", b], sig);
                  heldButtons.delete(b);
                }
                break;
              case "drag":
                ...
                sig.throwIfAborted();
                heldButtons.add("1");
                await input(["mousedown", "1"], sig);
                for (let i = 1; i <= 10; i++) {
                  await delay((a.durationMs ?? 200) / 10, undefined, { signal: sig });
                  await input(["mousemove", String(Math.round(a.x + ((a.toX - a.x) * i) / 10)), String(Math.round(a.y + ((a.toY - a.y) * i) / 10))], sig);
                }
                await input(["mouseup", "1"], sig);
                heldButtons.delete("1");
                break;
              case "scroll":
                for (const [delta, negative, positive] of [
                  ...
                  for (let i = 0; i < Math.ceil(Math.abs(delta) / 100); i++) {
                    sig.throwIfAborted();
                    await input(["click", delta < 0 ? negative : positive], sig);
                  }
                }
                break;
              case "type":
                for (const character of a.text) {
                  sig.throwIfAborted();
                  const k = character === "\n" ? "Return" : character === "\t" ? "Tab" : `U${character.codePointAt(0)!.toString(16).padStart(4, "0")}`;
                  heldKeys.add(k);
                  await input(["keydown", k], sig);
                  await input(["keyup", k], sig);
                  heldKeys.delete(k);
                }
                break;
              case "key":
                for (const key of a.key.split("+")) {
                ...
                break;
              case "wait":
                await delay(a.durationMs, undefined, { signal: sig });
                break;
              case "navigate":
                throw new ComputerUseError("unsupported", "Navigation belongs to browser targets, not native desktop");
            }
          } finally {
            await releaseInput();
          }
