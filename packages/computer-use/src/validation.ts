...
export const actionSchema = z.discriminatedUnion('type', [
 ...
 z.object({type:z.literal('drag'),x:coordinate,y:coordinate,toX:coordinate,toY:coordinate,durationMs:z.number().int().min(1).max(2000).optional()}).strict(),
 ...
 z.object({type:z.literal('key'),key:z.string().regex(/^(?:(?:Control|Alt|Shift|Meta)\+){0,4}(?:[a-zA-Z0-9]|Enter|Tab|Escape|Backspace|Delete|ArrowUp|ArrowDown|ArrowLeft|ArrowRight|Home|End|PageUp|PageDown|Space|F(?:[1-9]|1[0-2]))$/)}).strict(),
 z.object({type:z.literal('navigate'),url:z.string().url().max(4096).refine(s=>['http:','https:'].includes(new URL(s).protocol))}).strict(),
