...
export function getToolDefinitions(service: ComputerUseService, principal: Principal): ToolDefinition[] {
  ...
  const define = <T extends z.ZodRawShape>(
    name: string,
    description: string,
    inputSchema: T,
    fn: (input: z.infer<z.ZodObject<T>>, signal?: AbortSignal) => Promise<unknown> | unknown,
  ): ToolDefinition => ({
    name,
    description,
    inputSchema,
    async handler(raw, context) {
      try {
        ...
        if (result && typeof result === "object" && "frame" in result) {
          const { frame, ...metadata } = result as import("./contracts.js").Observation;
          // Fence again across the asynchronous service-to-transport boundary.
          ...
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify({ ...metadata, frame: { width: frame.width, height: frame.height, capturedAt: frame.capturedAt, url: frame.url } }),
              },
        ...
        const code = error instanceof ComputerUseError ? error.code : error instanceof z.ZodError ? "invalid_request" : "driver_error";
        return { isError: true, content: [{ type: "text", text: JSON.stringify({ error: code }) }] };
      }
    },
  });
  return [
    define("computer_status", "Redacted owner-scoped session status (no screenshots or leases).", { sessionId: z.string().uuid().optional() }, (input) =>
      service.status(p, input.sessionId),
    ),
    define(
      "computer_probe",
    ...
    define(
      "computer_open",
      "Open an enabled authorized target and acquire exclusive control. Observe before input.",
      { targetId: z.string().min(1).max(256) },
      (input, sig) => service.open(p, input.targetId, sig),
    ),
    ...
    define("computer_stop", "Immediately fence session input/capture and detach. Native applications remain open.", { sessionId: z.string().uuid() }, (input) =>
      service.stop(p, input.sessionId),
    ),
    define("computer_revoke", "Irreversibly revoke this session; no cached or future screenshots.", { sessionId: z.string().uuid() }, (input) =>
      service.revoke(p, input.sessionId),
    ),
  ];
}
