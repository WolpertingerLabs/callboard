...
/** Applied browser state only; dates are absolute instants, not local input strings. */
...
/** Kept self-contained so the daemon can run the identical regex work in a worker. */
export function filterChatRows<T extends { folder: string; displayFolder?: string; updated_at: string }>(
  rows: T[],
  filters: ChatFilters,
): { rows: T[]; warnings: string[] } {
  let result = rows;
  const warnings: string[] = [];
  for (const key of ["directoryInclude", "directoryExclude"] as const) {
    const field = filters[key];
    if (!field.active || !field.value) continue;
    try {
      const regex = new RegExp(field.value, "i");
      result = result.filter((row) => regex.test(row.displayFolder || row.folder) === (key === "directoryInclude"));
    } catch {
      warnings.push(`Ignored invalid ${key} regex`);
    }
  }
  for (const key of ["dateMin", "dateMax"] as const) {
    ...
    result = result.filter((row) => (key === "dateMin" ? new Date(row.updated_at).getTime() >= time : new Date(row.updated_at).getTime() <= time));
  }
