...
function ids(req: Request): { chatId: string; sessionId: string } {
  ...
  if (!idPattern.test(chatId) || (sessionId && !idPattern.test(sessionId))) throw Object.assign(new Error("Invalid control identifier"), { code: "invalid_request" });
...
function handle(fn: (req: Request) => Promise<unknown>) {
  return async (req: Request, res: Response) => {
    try {
      ...
      const status = code === "not_found" ? 404 : code === "invalid_request" ? 400 : ["denied", "approval_required"].includes(code) ? 403 : ["lease_conflict", "stale_generation", "stopped", "revoked"].includes(code) ? 409 : 503;
...
computerUseRouter.get("/:chatId/status", handle(async (req) => {
  const { chatId } = ids(req);
  return (await getComputerUseHost()).status(chatId);
}));
computerUseRouter.post("/:chatId/open", handle(async (req) => {
  const { chatId } = ids(req);
  const kind = req.body?.kind;
  if (kind !== "browser" && kind !== "desktop" && kind !== "native" && kind !== "native-desktop") throw Object.assign(new Error("Choose browser or native desktop explicitly"), { code: "invalid_request" });
  return { session: await (await getComputerUseHost()).open(chatId, kind === "browser" ? "browser" : "desktop") };
}));
computerUseRouter.post("/:chatId/:sessionId/observe", handle(async (req) => {
  const { chatId, sessionId } = ids(req);
  return (await getComputerUseHost()).observe(chatId, sessionId);
}));
computerUseRouter.post("/:chatId/:sessionId/action", handle(async (req) => {
  const { chatId, sessionId } = ids(req);
...
for (const operation of ["approve", "takeover", "resume", "stop", "revoke"] as const) {
  computerUseRouter.post(`/:chatId/:sessionId/${operation}`, handle(async (req) => {
    const { chatId, sessionId } = ids(req);
