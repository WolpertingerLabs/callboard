...
describe("computer control origin boundary", () => {
  ...
  it("requires same-origin browser mutations, including an Origin header", () => {
    for (const headers of [{ host: "callboard.local" }, { host: "callboard.local", origin: "null" }, { host: "callboard.local", origin: "https://evil.example" }, { host: "callboard.local", origin: "https://callboard.local", "sec-fetch-site": "cross-site" }]) {
      const { res, next } = run("POST", headers); expect(res.status).toHaveBeenCalledWith(403); expect(next).not.toHaveBeenCalled();
    }
  });
  ...
  it("never caches status or frame responses", () => {
    const { res, next } = run("GET", {}); expect(next).toHaveBeenCalledOnce(); expect(res.setHeader).toHaveBeenCalledWith("Cache-Control", "no-store, private");
  });
});
