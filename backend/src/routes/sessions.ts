...
sessionsRouter.get("/poll", (_req, res) => {
  ...
  /* #swagger.parameters['b'] = { in: 'query', required: false, type: 'string', description: "Client last-known daemon build id. Omit it, or send one that no longer matches, and the response carries `build`. The client uses a change in that value to offer a reload — its bundle is from before the daemon moved. The literal string 'unknown' means this daemon has no built frontend to identify. NOTE for hand-written callers: a build id contains a '+', which decodes as a space in a query string. URLSearchParams escapes it for you; curl does not, so use --data-urlencode or the echo will never match." } */
