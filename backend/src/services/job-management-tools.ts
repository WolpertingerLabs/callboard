...
- "job": run another job as a child run and wait for it. Fields: jobId (required — the child job; must exist, no self-reference), inputs ({ key: value } for the child's declared inputs; values support templating), outputs (array of keys harvested from the child's run-level outputs declaration — missing keys fail the step), timeoutHours (child run is cancelled on timeout), onTimeout ("fail" default or stepId), onFailure ("fail" default or stepId — child failed or was cancelled). The child definition is frozen when the step starts; nesting is bounded (depth 5). The child enforces its own limits; cancelling the parent cascades to the child.
