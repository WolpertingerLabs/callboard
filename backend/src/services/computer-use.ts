...
export interface HostPolicy { policy: ComputerUsePolicy; signature: string }
export function loadComputerUsePolicy(chatId: string): HostPolicy {
  ...
  try { metadata = JSON.parse(chat.metadata || "{}"); } catch { throw controlError("denied", "Chat permission metadata is unreadable"); }
...
export function controlError(code: string, message: string): Error & { code: string } { return Object.assign(new Error(message), { code }); }
...
interface Grant { chatId: string; signature: string; expiresAt: number; lease: Lease }
interface Pending { chatId: string; kind: ComputerTargetKind; signature: string; expiresAt: number; sessionId?: string; generation?: number; action?: unknown; execute?: (id: string) => Promise<unknown> }
...
export class ComputerUseHost {
  ...
  constructor(
    ...
    this.watchdog = setInterval(() => { void this.expire(); }, 1000);
    this.watchdog.unref();
  }
  ...
  private listen(chatId: string) {
    ...
    this.unsubscribers.set(chatId, this.service.subscribe(controlPrincipal(chatId, "human"), (event) => {
      const entries = this.events.get(chatId) ?? []; entries.push(event); this.events.set(chatId, entries.slice(-100));
    }));
  }
  private async expire() {
    for (const [id, grant] of this.grants) {
      let valid = grant.expiresAt > Date.now();
      try { valid = valid && this.readPolicy(grant.chatId).signature === grant.signature; } catch { valid = false; }
      if (!valid) {
  ...
  private presentation(value: SessionStatus) {
    return { ...value, id: value.sessionId, kind: value.kind === "browser" ? "browser" : "native", controller: value.controller === "none" ? null : value.controller, targetLabel: hostname(), target: hostname() };
  }
  async status(chatId: string) {
    const { policy } = this.readPolicy(chatId);
    await this.expire();
    const capabilities = await Promise.all((["browser", "desktop"] as const).map(async (kind) => {
  ...
  async open(chatId: string, kind: ComputerTargetKind) {
    ...
    if (current.policy.computerControl === "ask") {
      if ([...this.pending.values()].filter((p) => p.chatId === chatId).length >= 2) throw controlError("queue_full", "Resolve the existing target approval first");
      const id = randomUUID(); this.pending.set(id, { chatId, kind, signature: current.signature, expiresAt: Date.now() + 120_000 });
      return { id, sessionId: id, kind: kind === "browser" ? "browser" : "native", state: "pending_approval", controller: null, generation: 0, targetLabel: hostname() };
    }
  ...
  async approve(chatId: string, id: string, _generation?: unknown) {
    ...
    const current = this.readPolicy(chatId);
    if (!pending || pending.chatId !== chatId || pending.expiresAt <= Date.now() || pending.signature !== current.signature || current.policy.computerControl === "deny") throw controlError("denied", "Approval expired or its scope changed; enable the target again");
  ...
  private async openApproved(chatId: string, kind: ComputerTargetKind, current: HostPolicy) {
    ...
    if (this.service.status(controlPrincipal(chatId, "human")).some((s) => s.targetId === targetId(kind) && ["starting", "ready"].includes(s.state))) throw controlError("lease_conflict", "Stop this chat's existing target session first");
    this.listen(chatId); this.opening.set(key, current);
    try {
  ...
  private grant(chatId: string, id: string): Grant {
    ...
    if (grant.expiresAt <= Date.now() || this.readPolicy(chatId).signature !== grant.signature) {
      this.grants.delete(id); void this.service.revoke(controlPrincipal(chatId, "human"), id); throw controlError("revoked", "Control grant expired or permissions changed");
    }
    return grant;
  }
  ...
  async action(chatId: string, id: string, action: unknown, generation: unknown) {
    ...
    return this.presentation(await this.service.act(controlPrincipal(chatId, "human"), { sessionId: id, generation: grant.lease.generation, leaseId: grant.lease.leaseId, actionId: randomUUID(), action: action as Action }));
  }
  ...
  async resume(chatId: string, id: string, expectedGeneration?: unknown) {
    ...
    grant.lease = await this.service.resume(controlPrincipal(chatId, "human"), { sessionId: id, generation: grant.lease.generation, leaseId: grant.lease.leaseId });
    return this.presentation(grant.lease);
  }
  async stop(chatId: string, id: string, _generation?: unknown) {
    if (this.pending.get(id)?.chatId === chatId) { this.pending.delete(id); return { id, state: "stopped" }; }
    // Even denied/revoked owners may stop; ownership still enforced by the service.
    const result = await this.service.stop(controlPrincipal(chatId, "human"), id); this.grants.delete(id); return this.presentation(result);
  }
  async revoke(chatId: string, id: string, _generation?: unknown) {
    if (this.pending.get(id)?.chatId === chatId) { this.pending.delete(id); return { id, state: "revoked" }; }
    const result = await this.service.revoke(controlPrincipal(chatId, "human"), id); this.grants.delete(id); return this.presentation(result);
  }
  async requestAgentAction(chatId: string, id: string, generation: number, action: unknown, execute: (id: string) => Promise<unknown>) {
    const grant = this.grant(chatId, id); this.agentLease(chatId, id, generation);
    if (!action || typeof action !== "object" || Array.isArray(action) || JSON.stringify(action).length > 8192 || !["click", "move", "drag", "scroll", "key", "type", "navigate", "wait"].includes(String((action as { type?: unknown }).type))) throw controlError("invalid_request", "Action must be a bounded GUI operation");
    if ([...this.pending.values()].filter((p) => p.chatId === chatId).length >= 4) throw controlError("queue_full", "Resolve existing GUI action approvals first");
    const approvalId = randomUUID();
    this.pending.set(approvalId, { chatId, kind: uiKind(grant.lease.kind), signature: grant.signature, expiresAt: Date.now() + 120_000, sessionId: id, generation, action, execute });
    return { approvalRequired: true, approvalId, sessionId: id, instruction: "Wait for the human to confirm this action in the Computer Control panel, then observe the resulting state. Never repeat an unconfirmed mutation." };
  }
  ...
  async dispose() { clearInterval(this.watchdog); this.grants.clear(); this.pending.clear(); for (const off of this.unsubscribers.values()) off(); this.unsubscribers.clear(); await this.service.dispose(); }
}
...
export async function shutdownComputerUse(): Promise<void> { const pending = hostPromise; hostPromise = undefined; if (pending) await (await pending).dispose(); }
