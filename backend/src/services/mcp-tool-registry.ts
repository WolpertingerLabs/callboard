...
const CALLBOARD_TOOLS: McpToolDefinition[] = [
    ...
    parameters: [
      {
        name: "lifecycle",
        type: "enum",
        description: "Only cards in this lifecycle (default: all)",
        required: false,
  ...
  },
  {
    ...
    parameters: [
      ...
      },
      {
        ...
        type: "boolean",
        description: "Require the spawned session to call objective_complete before it counts as done (re-prompted if it ends without the call)",
        required: false,
      },
      {
        name: "role",
      ...
      },
      {
        ...
        required: false,
        enumValues: ["claude-code", "openrouter", "codex"],
      },
      { name: "model", type: "string", description: 'Model slug for provider="openrouter" or provider="codex"', required: false },
    ],
    serverName: "callboard-tools",
    serverLabel: "Callboard Tools",
    category: "platform",
  ...
  {
    name: "set_model_routing",
    qualifiedName: "mcp__callboard-tools__set_model_routing",
    description: "Replace the entire global Model Routing configuration (OpenRouter-only). Validated before saving.",
    parameters: [
      { name: "enabled", type: "boolean", description: "Whether model routing is available for new OpenRouter chats", required: true },
      { name: "classifierModel", type: "string", description: "OpenRouter slug/alias for the classifier call", required: true },
      { name: "classes", type: "array", description: "Task categories: [{ id, label, description }]", required: true },
      { name: "ranks", type: "array", description: "Quality/cost tiers: [{ id, label, order? }]", required: true },
      { name: "matrix", type: "object", description: "classId → rankId → model slug", required: false },
      { name: "defaultRankId", type: "string", description: "Default tier id", required: false },
      { name: "defaultClassId", type: "string", description: "Fallback class id when the classifier is uncertain", required: false },
    ],
    serverName: "callboard-tools",
    serverLabel: "Callboard Tools",
    category: "platform",
  ...
  {
    name: "update_model_routing",
    qualifiedName: "mcp__callboard-tools__update_model_routing",
    description: "Granularly patch the global Model Routing configuration (OpenRouter-only) without resending the whole thing. Validated before saving.",
    parameters: [
      ...
      { name: "removeRankIds", type: "array", description: "Rank ids to remove", required: false },
      { name: "setCells", type: "array", description: "Set/clear cells: [{ classId, rankId, model }] (empty model clears)", required: false },
    ],
    serverName: "callboard-tools",
    serverLabel: "Callboard Tools",
    category: "platform",
