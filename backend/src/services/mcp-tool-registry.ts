...
const CALLBOARD_TOOLS: McpToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Read-only chat search across every engine, with global stable pagination. Folder or repo scoped, with transcript grep, branch, agent and lineage filters. Visible scope uses the originating browser tab's live effective sidebar filters. Ignored directories are always excluded.",
    parameters: [
      ...
      { name: "folder", type: "string", description: "Exact working directory", required: false },
      {
        name: "repo",
        type: "string",
        description: "Repo root; expands to its worktrees, including removed ones. Each row reports repoSource for how it was admitted.",
        required: false,
      },
      {
        name: "branch",
        type: "string",
        description: "Recorded branch, falling back to the directory's live branch; each row reports branchSource",
        required: false,
      },
      { name: "agentAlias", type: "string", description: "Chats started by a specific agent", required: false },
      { name: "triggered", type: "boolean", description: "Automated (true) or manual (false) sessions", required: false },
      {
        name: "grep",
        type: "string",
        description: "Transcript content search, run last over the narrowed candidates. Each hit reports matchKind (engine semantics differ).",
        required: false,
      },
      { name: "rootChatId", type: "string", description: "Chats in the tree rooted at this chat (global, not folder-scoped)", required: false },
      { name: "parentChatId", type: "string", description: "Direct children of this chat (global, not folder-scoped)", required: false },
      ...
      { name: "sort", type: "enum", description: "Sort field (default: updated)", required: false, enumValues: ["updated", "created"] },
