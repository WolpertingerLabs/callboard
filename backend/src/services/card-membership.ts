...
export function createCardMembership(stored = listChatsSnapshot()) {
  ...
  for (const chat of stored) {
    ...
    for (const sid of new Set([chat.session_id, ...(Array.isArray(meta.session_ids) ? meta.session_ids : [])])) {
      ...
      const owners = ownersBySession.get(sid) ?? [];
      owners.push(chat);
      ownersBySession.set(sid, owners);
    }
  }
  ...
  const corpus = new Map<string, Chat>(
    stored.map((chat) => {
      const original = parseChatMetadata(chat.metadata);
      const normalized = nativeMetadata("", chat.session_id, original, false);
      return [chat.id, { ...chat, metadata: normalized === original ? chat.metadata : JSON.stringify(normalized) }];
    }),
  );
  ...
  const evidence = provider.nativeDiscoveryEvidence();
  const anchors = new Set(evidence.flatMap((entry) => (entry.meta.nativeAgent ? [entry.meta.nativeAgent.parentThreadId] : [])));
  for (const entry of evidence) {
    if (!entry.meta.nativeAgent && !anchors.has(entry.threadId)) continue;
    ...
    for (const chat of targets) {
      ...
      if (entry.meta.nativeAgent) verifiedParents.set(id, entry.meta.nativeAgent.parentThreadId);
  ...
  return {
    ...
    deferredNativeSessions: new Set(provider.nativeDiscoveryDeferredSessions ?? []),
