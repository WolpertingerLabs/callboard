 ...
 * card (and dismissing it from the inbox) must never bump updated_at, which
 * would resurface the chat as unread and reorder the sidebar.
 */
...
const { setChatCardMembership, setBoardDismissed, getChatCardId } = await import("./card-membership.js");
...
describe("setBoardDismissed", () => {
  it("dismisses without bumping updated_at", () => {
    const chat = chatFileService.createChat("/tmp/proj", "sess-3", "{}");
    const before = chatFileService.getChat(chat.id)!.updated_at;

    expect(setBoardDismissed(chat.id, true)).toBe(true);
    const after = chatFileService.getChat(chat.id)!;
    expect(after.updated_at).toBe(before);
    expect(JSON.parse(after.metadata).boardDismissed).toBe(true);
  });
});
