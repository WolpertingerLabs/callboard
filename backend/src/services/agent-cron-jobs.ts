...
const CONSOLIDATION_PROMPT = [
  ...
  "",
  "Read MEMORY.md first — you can't update what you haven't read. Then, in this order:",
  "",
  "1. **Remove** anything no longer true or no longer relevant, and merge entries that say the same thing.",
  "2. **Update in place** when a fact has changed — edit the existing entry, don't add a second one beside it.",
  "3. **Add** only what will still matter weeks from now and isn't already covered.",
  "",
  "Guidelines:",
  "- Keep it curated — MEMORY.md is your long-term memory, not a dump of everything",
  "- Reuse existing headings where they fit; drop headings left empty",
  "- Include dates for time-sensitive context",
  "- Preserve the existing structure and tone",
  "",
  "The journals keep the full record, so nothing is lost by trimming.",
  "",
  ...
  "",
  "Only update these if there is genuinely new, relevant information. Read each file first, and revise",
  "what's now wrong rather than appending a correction under it.",
  "",
  "## Step 4: Log Summary",
  "",
  ...
  "- Anything you pruned or merged (skip if there was nothing to prune)",
  "",
...
const DEFAULT_CRON_JOBS: Array<Omit<CronJob, "id">> = [
  ...
  },
  {
    ...
    status: "active",
    description:
      "Nightly review of journal entries. Prunes stale memory, revises existing entries in place, and distills new insights into MEMORY.md, updating SOUL.md, USER.md, TOOLS.md as needed.",
    action: {
      type: "start_session",
