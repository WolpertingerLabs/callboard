...
it("Cline 0.0.82 execute → runtime history → provider HTTP body retains image bytes and MIME", async () => {
  ...
  const fetchMock = vi.fn(async (_url: unknown, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ type: "error", error: { type: "invalid_request_error", message: "offline fixture" } }), {
      status: 400,
  ...
  const gateway = createGateway({
    providerConfigs: [{ providerId: "anthropic", apiKey: "offline-fixture", fetch: fetchMock as typeof fetch }],
      ...
      expect.objectContaining({
        ...
        content: expect.arrayContaining([
          expect.objectContaining({
            ...
            content: expect.arrayContaining([{ type: "image", source: { type: "base64", data: pixels, media_type: "image/png" } }]),
          }),
        ]),
      }),
    ]),
...
it("Cline runtime abort reaches the neutral handler's AgentToolContext signal", async () => {
  ...
  const { tools } = buildClineTools({
    ...
    version: "1",
    tools: [
      defineTool("cu_action", "offline pending call", {}, async (_args, context) => {
