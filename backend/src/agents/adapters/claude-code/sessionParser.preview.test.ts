...
describe("getFirstUserMessage — CLI plumbing is not a preview", () => {
  it("skips the isMeta caveat the CLI writes before a built-in command", () => {
    // Real shape: every built-in envelope is preceded by an isMeta
    // `<local-command-caveat>` paragraph. Without a meta filter that paragraph
    // *was* the sidebar preview for the whole chat.
    ...
    const path = transcript([{ ...userLine(caveat), isMeta: true }, userLine("<command-name>/login</command-name><command-args></command-args>")]);
    expect(getFirstUserMessage(path)).toBe("/login");
  });

  it("skips the isMeta resume nudge too", () => {
    const path = transcript([{ ...userLine("Continue from where you left off."), isMeta: true }, userLine("the real first message")]);
    expect(getFirstUserMessage(path)).toBe("the real first message");
  });

  it("does not project an entry that carries an image alongside the envelope", () => {
    // Matches parseMessages, so the two cannot disagree about what a command is.
    const path = transcript([
      userLine([
        { type: "text", text: "<command-name>/skill</command-name><command-args>look</command-args>" },
        { type: "image", source: { type: "base64", media_type: "image/png", data: "aaa=" } },
      ]),
    ]);
    expect(getFirstUserMessage(path)).toBe("<command-name>/skill</command-name><command-args>look</command-args>");
  });
});
