...
for (const engine of ["claude-code", "codex", "acp"] as const) describe(`${engine} native MCP relay`, () => {
  async function connect(spec: ToolServerSpec) {
    ...
    try { await client.connect(new StdioClientTransport({ command: config.command, args: config.args })); }
    catch (error) { await handle.close(); throw error; }
    return { client, close: () => handle.close() };
  }
  it("discovers canonical names and transports model-facing pixels unchanged", async () => {
    const { client, close } = await connect({ name: "computer_use", version: "1", tools: [defineTool("cu_observe", "fixture", {}, async () => ({ content }))] });
    try {
    ...
    } finally { await client.close(); await bounded(close()); }
  });
  it("propagates MCP cancellation/request ID and closes even an uncooperative pending call", async () => {
    ...
    const entered = new Promise<ToolCallContext>((resolve) => { resolveEntered = resolve; });
    const { client, close } = await connect({ name: "computer_use", version: "1", tools: [defineTool("cu_action", "pending fixture", {}, async (_args, context) => {
  ...
  it("turn teardown aborts pending calls without waiting for their handlers", async () => {
    let resolveEntered!: (context: ToolCallContext) => void;
    const entered = new Promise<ToolCallContext>((resolve) => { resolveEntered = resolve; });
    const { client, close } = await connect({ name: "computer_use", version: "1", tools: [defineTool("cu_action", "pending", {}, async (_args, ctx) => { resolveEntered(ctx!); return new Promise(() => {}); })] });
    ...
    } finally { await client.close(); await bounded(close()); await pending; }
  });
});

for (const engine of ["cline", "pi"] as const) it(`${engine} MCP-backed custom wrapper forwards cancellation/id and model-facing bytes`, async () => {
  const controller = new AbortController();
  let context: ToolCallContext | undefined;
  const spec = { name: "computer_use", version: "1", tools: [defineTool("cu_observe", "fixture", {}, async (_args, ctx) => { context = ctx; return { content }; })] };
  const result = engine === "cline"
